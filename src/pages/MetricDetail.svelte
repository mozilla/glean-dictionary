<script>
  ...
  function getSampleSQLQueryCode(metricType, table, columnName, additionalInfo) {
    if (metricType === "event") {
      ...
      <tr>
        ...
        <td>
          <div>
            ...
            {:else}
              (category=<code>"{metric.event_info.category}"</code>
              and name=<code>"{metric.event_info.name}"</code>)
            {/if}
            <CopyButton
              tooltipText="Copy the SQL code"
              textToCopy={getSampleSQLQueryCode(
                metric.type,
